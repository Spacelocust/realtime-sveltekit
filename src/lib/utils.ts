...
export const flyAndScale = (
  ...
  const styleToString = (styleToConvert: Record<string, number | string | undefined>): string => {
    return Object.keys(styleToConvert).reduce((str, key) => {
      if (styleToConvert[key] === undefined) return str;
      return `${str}${key}:${styleToConvert[key]};`;
    }, '');
  };
